...
Deno.test("LibDate initialisation no arguments ok", () => {
  ...
  assertEquals(today.toISOString().slice(0, 10), libDate.toISOString());
});

Deno.test("LibDate initialisation with string date", () => {
  let libDate;
  libDate = new LibDate("2021-01-01");
  assertEquals(libDate.toISOString(), "2021-01-01");
  ...
  assertEquals(libDate.toISOString(), "1999-12-31");
  ...
  assertEquals(libDate.toISOString(), "2001-12-31");
  libDate = new LibDate("31/12/2002");
  assertEquals(libDate.toISOString(), "2002-12-31");
  libDate = new LibDate("12-31-2003"); // US format mm/dd/yyyy
  assertEquals(libDate.toISOString(), "2003-12-31");
  libDate = new LibDate("12/31/2004");
  assertEquals(libDate.toISOString(), "2004-12-31");
});
...
Deno.test("LibDate getUnixTime ok", () => {
  const libDate = new LibDate("2021-01-01");
  assertEquals(libDate.getUnixTime(), 1609459200000);
});
